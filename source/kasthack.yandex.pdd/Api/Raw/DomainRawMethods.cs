...
namespace kasthack.yandex.pdd.RawMethods {
    ...
    public class DomainRawMethods : RawMethodsBase {
        ...
        public async Task<string> SetLogo(Stream source, string filename=null) {
            return await Context.ProcessRequestPostForm("domain/logo/set", new MultipartFormDataContent { MiscTools.StreamContent( filename??"file.png", source ) } ).ConfigureAwait( false );
        }

    }

    public class DomainsRawMethods : RawMethodsBase {
